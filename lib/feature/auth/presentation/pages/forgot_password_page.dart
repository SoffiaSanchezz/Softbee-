...
class _ForgotPasswordPageState extends ConsumerState<ForgotPasswordPage>
  ...
  void initState() {
    ...
    // Listener para actualizar el estado en el controlador
    ...
    ref.listen<ForgotPasswordState>(forgotPasswordControllerProvider, (previous, next) {
      if (next.emailSent || next.errorMessage != null) {
        _showResultModal(
          ...
          message:
              next.errorMessage ?? 'Enlace de recuperación enviado con éxito.',
          email: next.email,
        );
      }
    });
  ...
  Widget _buildForgotPasswordForm(
    ...
    // Escuchar el estado para el formulario
  ...
  Widget _buildSendButton({
    ...
    return Container(
      ...
      child: ElevatedButton(
        onPressed: (isLoading || isEmailSent) ? null : onPressed,
        style: ElevatedButton.styleFrom(
          backgroundColor: Colors.transparent,
          foregroundColor: Colors.white,
        ...
        child: isLoading
            ...
            : Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Icon(isEmailSent ? Icons.check_circle : Icons.send, size: 20),
                  const SizedBox(width: 10),
                  Text(
                    isEmailSent ? 'Enlace Enviado' : 'Enviar Enlace',
                    style: GoogleFonts.poppins(
                      fontSize: fontSize,
                      fontWeight: FontWeight.bold,
                      letterSpacing: 0.2,
